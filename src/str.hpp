...
namespace AustinUtils {
    ...
    public:
        ...
        void replace(str& s, usize start, usize len);

        template<typename T, std::enable_if_t<!std::is_same_v<T, str>, int> = 0>
        void replace(T& x, usize start, usize len) {
            str to_replace;
            to_replace.append(x);
        ...
        void copy(char* buf, const usize begin, const usize n = npos) const;

        NODISCARD usize find(const str& s, usize begin = 0, usize end = npos);

        NODISCARD usize rfind(const str& s, usize begin = 0, usize end = npos) const;
