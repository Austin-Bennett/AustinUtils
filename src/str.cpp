...
namespace AustinUtils {
    ...
    str::str(const char *c_str) {
        ...
        msize = slength+5;
        cstr = new char[msize];
        memcpy_s(cstr, msize, c_str, slength);
        cstr[slength] = '\0';
    }
    ...
    str::str(const str &s) {
        ...
        msize = s.msize;
        cstr = new char[msize];
        memcpy_s(cstr, msize, s.cstr, slength);
        cstr[slength] = '\0';
    }
    ...
    str& str::operator=(const str &other) {
        if (&other != this) {
            ...
            msize = other.msize;
            cstr = new char[msize];
            memcpy_s(cstr, msize, other.cstr, slength);
            cstr[slength] = '\0';
        }
        return *this;
    }
    ...
    void str::resize(usize n) {
        ...
        //copy the data
        memcpy_s(c_str, n, cstr, std::min(n, plen));
        c_str[slength] = '\0';
    ...
    void str::append(const str &s) {
        ...
        memcpy_s(&cstr[slength], s.slength, s.cstr, s.slength);
    ...
    void str::append(const char *s) {
        ...
        memcpy_s(&cstr[slength], len, s, len);

        slength += len;
        cstr[slength] = '\0';
    }

    void str::append(std::string s) {
        //check if we have enough room to append
        usize len = s.length();
        ...
        memcpy_s(&cstr[slength], len, s.c_str(), len);
    ...
    void str::insert(const str &s, usize pos) {
        ...
        memcpy_s(&cstr[pos], s.slength, s.cstr, s.slength);
        slength += s.slength;
        cstr[slength] = '\0';
    ...
    char* str::c_str() const {
        ...
        memcpy_s(ret, slength, cstr, slength);
        ret[slength] = '\0';
    ...
    void str::copy(char *buf, const usize begin, const usize n) const {
        ...
        memcpy_s(buf, std::min(slength-begin, n), &cstr[begin], std::min(slength-begin, n));
    }

    usize str::find(const str& s, const usize begin, usize end) {
    ...
    str str::substr(const usize start, const usize n) const {
        if (start > slength) throw Exception("Cannot access elements at " + std::to_string(start));
