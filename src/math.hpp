...
struct AUSTINUTILS v2 {
    ...
    friend logger& operator << (logger& lg, v2<T> self) {
        lg << "<" << self.x << ", " << self.y << ">";
        return lg;
    }
...
class AUSTINUTILS complex {
    ...
    friend logger& operator << (logger& lg, complex self) {
        if (self.is_real()) {
            lg << self.Real;
            return lg;
        }
        if (self.Imaginary < 0) {
            lg << self.Real << " - " << std::abs(self.Imaginary) << "i";
            return lg;
        }
        lg << self.Real << " + " << self.Imaginary << "i";
        return lg;
    }
